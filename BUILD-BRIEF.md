...
You are picking up a WellStack platform build mid-stream. You have no conversation history. Read this entire file before writing a single line of code.

## What You're Building

WellStack is a senior living property technology platform. Think Bloomberg terminal for property wellness — dark mode, institutional density, data-rich. It combines smart home device management (P1) with wellness intelligence (P2), intervention workflows (P3), and community health analytics (P4).

## What's Already Built

**P1 — Base Smart Home: COMPLETE.** The core operator dashboard exists: portfolio view, property view, unit detail, device cards (locks, thermostats, leak sensors), alerts page, access control, HVAC management, reports. Built with Next.js 14 App Router, TypeScript (strict), Tailwind CSS, shadcn/ui, Zustand, Framer Motion, Recharts, Sonner.

**P2 — Wellness Intelligence: PARTIALLY COMPLETE.** Several wellness cards exist in `components/wellness/`: IEQCard, WellnessScoreCard, FallDetectionCard, SleepEnvironmentCard, InterventionStatusCard (stub only), CommunityIntelCard (stub only). These are integrated into the Unit Detail view. Some are complete; some are stubs awaiting P3/P4.

## Your Job

**AUDIT THE CODEBASE FIRST.** Before writing anything:
1. Read the existing component structure, design system, and mock data patterns
2. Identify exactly what P2 components are complete vs. stub
3. Understand the routing structure, Zustand store patterns, and mock data file organization
4. Then build only what's missing — never rebuild what works

**KEY FILES TO READ FIRST:**
- `components/wellness/` — all existing P2 components
- `components/unit/UnitDetail.tsx` — how wellness cards are slotted in
- `lib/types/wellness.ts` — existing wellness type definitions
- `lib/mock/wellnessData.ts` — existing mock data patterns
- `lib/store/wellnessStore.ts` — state management patterns
- `components/alerts/AlertsPage.tsx` — P1 alert UI to extend (not replace)
- `app/(dashboard)/` — routing structure
- Any existing theme/globals CSS — match this exactly

## Critical Context

- **Dark mode. Institutional. Dense.** This will be shown to a 30-year multifamily veteran ($8B in transactions). It needs to feel like it was built by people who understand both wellness science and real estate finance.
- **Three demo scenario units:** Unit 204 (healthy, score 88), Unit 312 (watch, score 67), Unit 118 (intervention needed, score 42, fall event, isolation flag).
- **The Wellness Score is THE hero metric** — design it like a credit score.
- **Family Dashboard is a SEPARATE experience** — warmer, lighter, non-technical.
- **WELL Building Standard thresholds are real numbers** — use exactly what's in this spec.
- **Mock data must be consistent across all views.**

## Tech Stack (already installed)
Next.js 14 App Router · TypeScript strict · Tailwind CSS · shadcn/ui · Zustand · Framer Motion · Recharts · Sonner · Lucide React
